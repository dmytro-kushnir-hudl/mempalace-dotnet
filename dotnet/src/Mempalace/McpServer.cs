...
public static async Task RunAsync(McpToolContext ctx, CancellationToken ct = default)
        ...
        McpToolContext ctx, JsonNode? id, JsonNode? params_, CancellationToken ct)
    {
        ...
        JsonNode result;
        try { result = await DispatchAsync(ctx, name, args, ct); }
        catch (Exception ex) { result = new JsonObject { ["error"] = ex.Message }; }
    ...
    {
        string? S(string key) => args[key]?.GetValue<string>();
        int     I(string key, int def) => args[key] is { } v ? (int)v.GetValue<double>() : def;
        double  D(string key, double def) => args[key] is { } v ? v.GetValue<double>() : def;
        ...
        return name switch
        {
            ...
            "mempalace_add_drawer"      => await McpTools.AddDrawerAsync(ctx, S("wing")!, S("room")!, S("content")!, S("source_file") ?? "", S("added_by") ?? "mcp", ct),
            ...
            "mempalace_kg_add"          => McpTools.KgAdd(ctx, S("subject")!, S("predicate")!, S("object")!, S("valid_from"), S("valid_to"), D("confidence", 1.0), S("source_closet")),
            "mempalace_kg_invalidate"   => McpTools.KgInvalidate(ctx, S("subject")!, S("predicate")!, S("object")!, S("ended")),
    ...
    [
        ...
        Tool("mempalace_search",           "Semantic search with optional wing/room filter",
            new { query = Req(Str("Search query")), limit = Int("Max results (default 5)"), wing = Str("Wing filter"), room = Str("Room filter") }),
        Tool("mempalace_check_duplicate",  "Check if content already exists in the palace",
            new { content = Req(Str("Content to check")), threshold = Num("Similarity threshold 0–1 (default 0.9)") }),
        ...
        Tool("mempalace_add_drawer",       "File verbatim content into a wing/room",
            new { wing = Req(Str("Wing")), room = Req(Str("Room")), content = Req(Str("Verbatim content")), source_file = Str("Source file (optional)"), added_by = Str("Added by (default: mcp)") }),
        ...
        Tool("mempalace_kg_query",         "Query the knowledge graph for an entity's relationships",
            new { entity = Req(Str("Entity name")), as_of = Str("Date filter YYYY-MM-DD"), direction = Str("outgoing | incoming | both") }),
        Tool("mempalace_kg_add",           "Add a fact to the knowledge graph",
            new { subject = Req(Str("Subject entity")), predicate = Req(Str("Predicate")), @object = Req(Str("Object entity")), valid_from = Str("Valid from YYYY-MM-DD"), valid_to = Str("Valid to YYYY-MM-DD"), confidence = Num("Confidence 0–1"), source_closet = Str("Source drawer ID") }),
        Tool("mempalace_kg_invalidate",    "Mark a knowledge graph fact as no longer true",
            new { subject = Req(Str("Subject")), predicate = Req(Str("Predicate")), @object = Req(Str("Object")), ended = Str("End date YYYY-MM-DD") }),
        ...
        Tool("mempalace_diary_write",       "Write a diary entry in AAAK format",
            new { agent_name = Req(Str("Agent name")), entry = Req(Str("Diary entry (AAAK format)")), topic = Str("Topic tag") }),
        ...
        Tool("mempalace_extract_memories",  "Extract typed memories (decisions/milestones/problems/preferences/emotional) from text using heuristics — no LLM required",
            new { text = Req(Str("Text to extract from")), min_confidence = Num("Minimum confidence threshold 0–1 (default 0.3)") }),
        Tool("mempalace_detect_entities",   "Detect people and projects mentioned in text",
            new { text = Req(Str("Text to scan for entities")) }),
        Tool("mempalace_compress",          "Compress text to AAAK dialect (lossy summary — entities, topics, key quote, emotions, flags)",
            new { text = Req(Str("Text to compress")) }),
    ];

    // ── Schema helpers ────────────────────────────────────────────────────────

    private static object Tool(string name, string desc, object props) => new
    {
    ...
    private static object Req(object schema) => schema;  // required marking is separate in JSON Schema
