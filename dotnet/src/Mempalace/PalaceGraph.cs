...
public static class PalaceGraph
{
        ...
        BuildFromData(IEnumerable<Dictionary<string, object?>?> metadatas)
    {
        ...
        var nodes = roomData.ToDictionary(
            ...
            kv => new RoomNode(
                kv.Key,
                kv.Value.Wings.OrderBy(w => w).ToList(),
                kv.Value.Halls.OrderBy(h => h).ToList(),
                kv.Value.Count,
                kv.Value.Dates.OrderDescending().Take(5).ToList()));
    ...
    {
        var tunnelRooms = nodes.Values.Count(n => n.Wings.Count >= 2);
